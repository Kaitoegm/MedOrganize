...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const conclusaoSons = [
        'assets/sounds/task_complete/03-amerie-1-thing-audiotrimmer.mp3',
        'assets/sounds/task_complete/conclu.mp3',
        'assets/sounds/task_complete/cuteyhoney.mp3',
        'assets/sounds/task_complete/daddys-home.mp3',
        'assets/sounds/task_complete/not-cute-anymore.mp3',
        'assets/sounds/task_complete/pac-man-x-ms-pac-man-sugar-crush.mp3',
        'assets/sounds/task_complete/task_done.mp3',
        'assets/sounds/task_complete/romanceeeeeeeeeeeeee.mp3',
        'assets/sounds/task_complete/tmpdbnm_5a3_Sn8alVQ.mp3',
        'assets/sounds/task_complete/yeah-boiii-i-i-i.mp3'
    ];
    ...
    const devSkipFocusBtn = document.getElementById('dev-skip-focus-btn');
    ...
    function addTask(text) {
        ...
        renderTasks();
        playAudio('assets/sounds/task_create.mp3');
    ...
    function toggleTask(id, clickEvent = null) {
        ...
        if (task) {
            ...
            if (!wasCompleted) {
                playAudio('assets/sounds/task_check.mp3');
                ...
                playAudio('assets/sounds/task_uncheck.mp3');
                addTokens(-10, targetPos);
                recordChecklistTokenEarned(-10);
    ...
    function showExplanation(stepId) {
        ...
        if (exp) {
            ...
            playAudio('assets/sounds/gacha_spin.mp3');
        }
    }
    ...
    function renderErrors() {
        ...
        displayErrors.forEach((err, index) => {
            ...
            if (!err.resolved) {
                ...
                const deleteBtn = document.createElement('button');
                deleteBtn.className = 'btn-delete-error';
                deleteBtn.title = 'Excluir dúvida';
                deleteBtn.innerHTML = `<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="color: #c94b4b; display: block;"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>`;
                deleteBtn.addEventListener('click', () => deleteError(err.id));
        ...
        const tickHandler = () => {
            ...
            if (currentStage === 'foco') {
                ...
                if (timeLeft === 0 && !focusAlarmPlayed) {
                    ...
                    playAudio('assets/sounds/pomodoro_end.mp3');
                    addTokens(25);
    ...
    function triggerCountdown(val) {
        ...
        playAudio('assets/sounds/countdown_tick.mp3');
    }
    ...
    function transitionToFoco() {
        ...
        stopAmbiance();
        playAudio('assets/sounds/focus_start/undertale-battle-start.mp3');
    ...
    function transitionToPausa() {
        ...
        startActiveTimer(() => {
            ...
            playAudio('assets/sounds/break_end/cat-alarm.mp3');
            addTokens(5);
    ...
    focusPauseBtn.addEventListener('click', () => {
        if (timerRunning) {
            ...
            playAudio('assets/sounds/timer_pause.mp3');
            fullscreenFocus.classList.add('timer-paused');
            freezeActiveElements();
        } else {
            fullscreenFocus.classList.remove('timer-paused');
            unfreezeActiveElements();
            playAudio('assets/sounds/timer_resume.mp3');
            let nextAction = null;
            if (currentStage === 'preparo') {
                nextAction = () => {
                    countdownOverlay.classList.remove('active');
    ...
    // DEV SKIP BUTTON
    if (devSkipFocusBtn) {
        devSkipFocusBtn.addEventListener('click', () => {
            timeLeft = 1;
            if (timerRunning) {
    ...
    focusCancelBtn.addEventListener('click', () => {
        if (confirm("Deseja mesmo desistir desta sessão de foco?")) {
            playAudio('assets/sounds/pomodoro_quit2.mp3');
            exitFocusOverlay();
        }
    });
    ...
    function exitFocusOverlay() {
        ...
        stopAmbiance();
        stopBreakMode();
        // Lofi Radio persists! (do not clear iframes here)
    ...
    function checkDailyErrorReset() {
        const today = getFormattedToday();
        const lastReset = localStorage.getItem('med_cozy_last_error_reset_date') || '';
        if (lastReset !== today) {
            errorLogs = [];
            localStorage.setItem('med_cozy_errors', JSON.stringify(errorLogs));
            localStorage.setItem('med_cozy_last_error_reset_date', today);
        }
    }
    ...
    // --- Lofi Radio Drag & Drop ---
    function initMiniPlayerDrag() {
        const panel = document.getElementById('music-player-panel');
        const handle = document.getElementById('music-panel-drag-handle');
        ...
        function dragStart(e) {
            ...
            // Ignore if clicking on a button inside header
            ...
            // Use offsetLeft/offsetTop to get coordinates relative to the transformed parent
            // instead of getBoundingClientRect() which returns screen coordinates.
            ...
            // Override bottom/right to allow smooth left/top dragging
            panel.style.bottom = 'auto';
            panel.style.right = 'auto';
            panel.style.left = initialX + 'px';
            panel.style.top = initialY + 'px';
            panel.style.transition = 'none'; // remove shadow transition while dragging
        }
        ...
        function dragEnd(e) {
            ...
            panel.style.transition = 'box-shadow 0.3s ease'; // restore
        }
    }
    ...
    function startBreakAmbiance() {
        ...
        // Use the existing ambiance system — pick 'rain' by default
